...
pub async fn start_torrent_download(magnet_link: String) -> Result<i64, Box<dyn std::error::Error>> {
    ...
    let id = if start_torrent_res["id"].as_i64().is_none() {
      return Err(Box::<dyn std::error::Error>::from("Missing torrent id".to_string()));
    } else {
