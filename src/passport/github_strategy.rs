...
pub fn github_auth_source() -> actix_web::Scope {
	let passport = generate_github_passport();
	let passport_clone = Arc::new(RwLock::new(passport));

	web::scope("")
	.app_data(Data::new(passport_clone.clone()))
	.route("/redirect/github", web::get().to(authenticate_github))
	.route("/auth/github", web::get().to(github))
}
